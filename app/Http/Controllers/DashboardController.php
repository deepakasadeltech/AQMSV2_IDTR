...
class DashboardController extends Controller
{
   protected $setting;
    ...
    public function index()
    {
	 ...
	 //---======================payfine-&-Court-work==================================---------
	 'ppatient_list_doctorwise' =>$this->setting->PgetPatientListDoctorWise(Auth::user()->id, Auth::user()->department_id),
	 'rpatient_list_doctorwise' =>$this->setting->RgetPatientListDoctorWise(Auth::user()->id, Auth::user()->department_id),
	  //---================================================================
	  'totaloldtoken' => $this->setting->getTotalOldToken(),
	  'totalnewtoken' => $this->setting->NgetTotalOldToken()
	  //----=========================================-----
	...
	{  $totaloldtoken = $this->setting->getTotalOldToken();
		...
		$call = $queue->call()->create([
			...
			//'newbarcode' => $queue->newbarcode,
			'checkingCounter' => $queue->checkingCounter,
			'token' => $queue->token,
...
public function scannerDirectCall(Request $request)
    { 
		...
		$queueByPriority = $this->setting->NgetNextTokenByPriority($department);
		$inputbarcode = $request->newbarcode; 
		 $queue = Queue::find($queueByPriority[0]['id']);
		 if($inputbarcode == '' ){
			flash()->warning('Please Scan Your Barcode');
            return redirect()->route('dashboard');
		}
		if($queue->newbarcode !== $inputbarcode ){
			flash()->warning('Your Barcode Not Matched');
            return redirect()->route('dashboard');
		}else{
			flash()->success('Token Called');
			$newbarcode = $queue->newbarcode;
        ...
        return redirect()->route('dashboard');
		//}else{
       flash()->warning('No Token Available for Called');
        return redirect()->route('dashboard');
		//}
