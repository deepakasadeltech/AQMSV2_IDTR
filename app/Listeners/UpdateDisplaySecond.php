...
use App\Events\TokenCalledSecond;
...
class UpdateDisplaySecond
{
    ...
    public function handle(TokenCalledSecond $event)
    {    
        ...
        for ($i=0;$i<1;$i++) {
            ...
            $data[$i]['number'] = (isset($calls[$i]))?(($calls[$i]->department->letter!='')?$calls[$i]->department->letter.''.$calls[$i]->nt_number:$calls[$i]->nt_number):'NIL';
            $data[$i]['call_number'] = (isset($calls[$i]))?(($calls[$i]->department->letter!='')?$calls[$i]->department->letter.' '.$calls[$i]->nt_number:$calls[$i]->nt_number):'NIL';
			...
			$data[$i]['timeslot'] = (isset($calls[$i]))?$calls[$i]->timeslot:'NIL';
			$data[$i]['checkingCounter'] = (isset($calls[$i]))?$calls[$i]->checkingCounter:'NIL';
        }
		$data2 = [];
		...
		if(!empty($final_arr))
		{
			...
			foreach($final_arr as $d1)
			{
				...
				foreach($d1 as $d2)
				{    
					if($d2[0]['checkingCounter']=='R'){$heading = 'Court Work';}elseif($d2[0]['checkingCounter']=='P'){$heading = 'Pay Fine';}
					...
					$html .='<thead><tr><th>टोकन संख्या</th><th>प्रशिक्षण समय</th><th>जाँच काउंटर</th><th>कार्य</th></tr></thead>';
					$html .='<tbody>';
					foreach($d2 as $d3) {
						if($d3['view_status'] == 1) { 
						$html .='<tr>';
						$html .='<td id="">'.'<span class="patcurrentstatus"></span>'.$d3['call_number'].'</td>';
						$html .='<td id="">'.$d3['timeslot'].'</td>';
					if($d3['checkingCounter']=='R'){$html .='<td id="">'.'Court Work'.'</td>';}elseif($d3['checkingCounter']=='P'){$html .='<td id="">'.'Pay Fine'.'</td>';}else{$html .='<td id="">'.' '.'</td>';}
        ...
        file_put_contents(base_path('assets/files/displaysecond'), json_encode($data));
    }
}
