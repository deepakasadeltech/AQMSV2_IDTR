        ...
        <div class="container">
            <div class="row">
                <div class="col s12 m12 l12">
                    <h5 class="breadcrumbs-title col s5" style="margin:.82rem 0 .656rem">{{ trans('messages.mainapp.menu.missedprint') }}</h5>
                    <ol class="breadcrumbs col s7 right-align">
                        <li><a href="{{ route('dashboard') }}">{{ trans('messages.mainapp.menu.dashboard') }}</a></li>
                        <li class="active">{{ trans('messages.mainapp.menu.missedprint') }}</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="col s12 m12">
                <div class="card">
                    ...
                    <table id="token_detail" class="display" cellspacing="0">
                        ...
                        <tbody>
                            @foreach($tokenreprint as $tokendetail)
                           ...
                           <td>@if($tokendetail->nt_number==null) No New @else {{$tokendetail->nt_number}} @endif</td>
                           <td>
                            @if($tokendetail->nt_number==null) First Token @else Second Token @endif
                           ...
                           <td>
                           <a style="font-size:10px; margin:0px 4px; padding:5px 10px; line-height:inherit; height:auto;" href="{{url('/calls/rePrintToken')}}/{{ $tokendetail->id }}" class="waves-effect waves-light btn green" >Re-print Token</a>
...
@if(session()->get('timeslot') != '')
<tr> <td style="padding:4px; border:1px solid #555;">BATCH TIME <span style="float:right;">:</span></td>  <td style="padding:4px; border:1px solid #555;">{{ session()->get('timeslot') }}</td> </tr>
@else  @endif
