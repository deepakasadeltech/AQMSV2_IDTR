...
<input type="text" name="audio_last_id" id="audio_last_id" value="<?php echo $last_id; ?>" />
	...
	<div id="callarea1">
 <!------------------------------>
  <div style="display:none;" id="add_dynamic_slider1"></div>
 <!------------------------------->   
...
<?php }
     ...
     <li> <div class="datetimeglobal_time"><span>Plaese Wait...</span><span>No Token Called</span><span><?php date_default_timezone_set('Asia/Kolkata'); echo date("l"); ?></span><span><?php date_default_timezone_set('Asia/Kolkata'); echo date("d.m.Y"); ?></span> <span class="gtime"> <?php date_default_timezone_set('Asia/Calcutta');$h = date('h'); $a = $h >= 12 ? 'PM' : 'AM';
...
function liftTable() {
    ...
    $.ajax({
        //type: 'GET',
        url: "<?php echo e(route('all_display')); ?>",
        data: data,
        success: function(data) {
            console.log(data);
            $('#add_dynamic_slider1').html(data);
        },
    });
}
 //---------------------------------------------------- 
